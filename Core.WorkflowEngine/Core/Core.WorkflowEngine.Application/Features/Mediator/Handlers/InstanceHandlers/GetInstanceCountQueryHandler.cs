...
{
    public class GetInstanceCountQueryHandler : IRequestHandler<GetInstanceCountQuery, GetInstancesCountQueryResult>
    {
        ...
        public async Task<GetInstancesCountQueryResult> Handle(GetInstanceCountQuery request, CancellationToken cancellationToken)
        {
            DBQueryOptions<Instance> options = new DBQueryOptions<Instance>();

            int dataCount = await _repository.GetAllDataCountAsync(options);

            return _mapper.Map<GetInstancesCountQueryResult>(dataCount);
        }
    }
}
